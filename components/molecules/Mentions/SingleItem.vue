<template lang="pug">
.xm-mentions-single-item(:class="isOriginalMessage && 'has-reply'")
  .row
    XAAvatar.mr-4(:image="avatar", :name="username", size="48px")
    .details.col
      .row.justify-between.user-details
        .username {{ isOriginalMessage ? repliedToUsername : username }}
        .date {{ isOriginalMessage ? repliedToDate : date }}
      XMMentionsMedias(
        v-if="isOriginalMessage && repliedToPhotoURLs.length",
        :medias="repliedToPhotoURLs"
      )
      .reply.row(v-if="showReply")
        .reply-media(v-if="repliedToPhotoURLSmall"): img(
          :src="repliedToPhotoURLSmall"
        )
        .col
          .reply-user {{ repliedToUsername }}
          .reply-text(v-html="repliedText")
      XMMentionsMedias(
        v-if="!isOriginalMessage && photoURLs.length",
        :medias="photoURLs"
      )
      ._message(v-html="isOriginalMessage ? repliedText : text")
</template>
...
export default {
  ...
  props: {
    ...
    isOriginalMessage: { type: Boolean, default: false },
  },
  computed: {
    user() {
    ...
    repliedToDate() {
      const date = this.repliedTo?.createdAt || new Date()
      const today = new Date()
      const isCurrentDate = moment(today).isSame(date, 'day')
      const days = moment(today).diff(date, 'days')
    ...
    repliedToPhotoURLs() {
      return this.repliedTo?.PhotoURLs || []
    },
